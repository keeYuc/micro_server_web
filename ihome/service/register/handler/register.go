package handler

import (
	"context"

	log "github.com/micro/micro/v3/service/logger"

	register "register/proto"
)

type Register struct{}

// Call is a single request handler called via client.Call or the generated client code
func (e *Register) Call(ctx context.Context, req *register.Request, rsp *register.Response) error {
	log.Info("Received Register.Call request")
	rsp.Msg = "Hello " + req.Name
	return nil
}

// Stream is a server side stream handler called via client.Stream or the generated client code
func (e *Register) Stream(ctx context.Context, req *register.StreamingRequest, stream register.Register_StreamStream) error {
	log.Infof("Received Register.Stream request with count: %d", req.Count)

	for i := 0; i < int(req.Count); i++ {
		log.Infof("Responding: %d", i)
		if err := stream.Send(&register.StreamingResponse{
			Count: int64(i),
		}); err != nil {
			return err
		}
	}

	return nil
}

// PingPong is a bidirectional stream handler called via client.Stream or the generated client code
func (e *Register) PingPong(ctx context.Context, stream register.Register_PingPongStream) error {
	for {
		req, err := stream.Recv()
		if err != nil {
			return err
		}
		log.Infof("Got ping %v", req.Stroke)
		if err := stream.Send(&register.Pong{Stroke: req.Stroke}); err != nil {
			return err
		}
	}
}
